...
#define	RES_SND_KEY_COLLECTED	(RES_SND_BASE + SND_KEY_COLLECTED)
...
/* The active SFX theme name (e.g. "MS", "LINK", or "" / "Tile World") */
static char current_sfx_theme[128] = "";
static int g_stop_bgm_on_level_complete = 0;

int get_stop_bgm_on_level_complete(void)
{
    return g_stop_bgm_on_level_complete;
}
...
void setsfxtheme(const char *theme_name)
{
    if (!theme_name) theme_name = "";
    strncpy(current_sfx_theme, theme_name, sizeof current_sfx_theme - 1);
    current_sfx_theme[sizeof current_sfx_theme - 1] = '\0';
...
static void initresourcedefaults(void)
{
    g_stop_bgm_on_level_complete = 0;
    strcpy(allresources[Ruleset_None][RES_IMG_TILES].str, "atiles.bmp");
...
/* Parse a specific rc file path, storing the values in the allresources array. */
static int parse_rc_file_path(char const *fullpath)
{
    resourceitem	item;
    fileinfo		file = {0};
    char		buf[256];
    ...
    if (!fileopen(&file, fullpath, "r", NULL))
	return FALSE;
	...
	for (i = sizeof rclist / sizeof *rclist - 1 ; i >= 0 ; --i)
		...
		break;
	if (i < 0) {
	    if (!strcmp(name, "stopbgmonlevelcomplete") || !strcmp(name, "stopbgmonwin") || !strcmp(name, "stopbgm")) {
		if (atoi(item.str) > 0 || !strcasecmp(item.str, "true") || !strcasecmp(item.str, "yes") || !strcasecmp(item.str, "on")) {
		    g_stop_bgm_on_level_complete = 1;
		} else {
		    g_stop_bgm_on_level_complete = 0;
		}
		continue;
...
/* Iterate through the lines of the rc file(s), storing the values in the
 ...
 * the ruleset-specific entries. FALSE is returned if the base rc file
 * could not be opened.
 */
static int readrcfile(void)
{
    char fullpath[PATH_MAX + 1];
    combinepath(fullpath, resdir, "rc");
    int res = parse_rc_file_path(fullpath);

    if (current_sfx_theme[0] && strcasecmp(current_sfx_theme, "Tile World") != 0) {
        char rel[256];
        snprintf(rel, sizeof rel, "sfx/%s/rc", current_sfx_theme);
        combinepath(fullpath, resdir, rel);
        parse_rc_file_path(fullpath);
    }
    return res;
}
...
static int loadimages(void)
{
    ...
    f = FALSE;
    path = getpathbuffer();
    char const *tileset_name = resources[RES_IMG_TILES].str;
    ...
    __android_log_print(ANDROID_LOG_INFO, "tworld", "loadimages: ruleset=%d tileset=%s path=%s size=%ld", currentruleset, tileset_name, path, size);
...
static int loadsounds(void)
{
	...
	if (*resources[RES_SND_BASE + n].str) {
	    if (current_sfx_theme[0] && strcasecmp(current_sfx_theme, "Tile World") != 0) {
		char rel[256];
		snprintf(rel, sizeof rel, "sfx/%s/%s", current_sfx_theme, resources[RES_SND_BASE + n].str);
		combinepath(path, resdir, rel);
	...
	if (!f && resources != globalresources
	    ...
	    if (current_sfx_theme[0] && strcasecmp(current_sfx_theme, "Tile World") != 0) {
		char rel[256];
		snprintf(rel, sizeof rel, "sfx/%s/%s", current_sfx_theme, globalresources[RES_SND_BASE + n].str);
		combinepath(path, resdir, rel);
		f = loadsfxfromfile(n, path);
	    }
	    if (!f) {
...
/* Return the TileImages filename currently set for the given ruleset.
 * Falls back to the Ruleset_None default if not overridden.
 */
const char *gettilesetting(int ruleset)
{
    return "atiles.bmp";
}

/* Update the in-memory TileImages value for the given ruleset.
 * Must be called alongside writing the rc file so loadgameresources picks it up.
 */
void settilesetting(int ruleset, const char *filename)
{
    if (ruleset < 0 || ruleset >= Ruleset_Count) return;
    strncpy(allresources[ruleset][RES_IMG_TILES].str, filename,
            sizeof allresources[ruleset][RES_IMG_TILES].str - 1);
    allresources[ruleset][RES_IMG_TILES].str[
        sizeof allresources[ruleset][RES_IMG_TILES].str - 1] = '\0';
}
