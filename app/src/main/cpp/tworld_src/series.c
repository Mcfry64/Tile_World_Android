			  ...
			  mapfileinfo **mflist, int *mfcount)
{
    ...
    memset(&s, 0, sizeof s);
    LOGI("getseriesfiles: preferred=%s", preferred ? preferred : "NULL");
	...
	*seriesdir = '\0';
	...
	if (preferred && *preferred) {
	    for (n = 0 ; n < s.count ; ++n) {
		char const *p = strrchr(s.list[n].name, '.');
		int len = p ? (int)(p - s.list[n].name) : (int)strlen(s.list[n].name);
		LOGI("getseriesfiles: checking %s against %s (prefix len=%d)", s.list[n].name, preferred, len);
		if (!strcmp(s.list[n].name, preferred) ||
		    (!strncmp(s.list[n].name, preferred, len) && (int)strlen(preferred) == len)) {
		    LOGI("getseriesfiles: MATCHED %s", s.list[n].name);
		    s.list[0] = s.list[n];
		    s.count = 1;
		    n = 0;
		    break;
	    ...
	    if (n == s.count) {
		LOGI("getseriesfiles: NO MATCH for %s in %d files", preferred, s.count);
