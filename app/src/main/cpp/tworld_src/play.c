...
int isoutoftime(void) {
    ...
    return (state.game && state.game->time && timeleft <= 0);
}
...
static int g_game_speed_percent = 100;

void android_set_game_speed(int percent)
{
    if (percent < 10) percent = 10;
    if (percent > 300) percent = 300;
    g_game_speed_percent = percent;

    if (state.ruleset != Ruleset_None) {
        int base_ms = (state.ruleset == Ruleset_MS) ? 1100 : 1000;
        settimersecond(base_ms * mudsucking * 100 / g_game_speed_percent);
    }
}
...
static int setrulesetbehavior(int ruleset) {
    ...
    switch (ruleset) {
        case Ruleset_Lynx:
            ...
            settimersecond(1000 * mudsucking * 100 / g_game_speed_percent);
            break;
        case Ruleset_MS:
            ...
            settimersecond(1100 * mudsucking * 100 / g_game_speed_percent);
            break;
        default:
...
int get_current_level_number(void) {
    if (state.game) return state.game->number;
    return 0;
}

char const* get_current_level_name(void) {
    if (state.game) return state.game->name;
    return "";
}

char const* get_current_level_author(void) {
    if (state.game) return state.game->author;
    return "";
}
