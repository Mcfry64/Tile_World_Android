...
/* Return the TileImages filename currently configured for the given ruleset. */
extern const char *gettilesetting(int ruleset);

/* Update the in-memory TileImages value for a ruleset (call before loadgameresources). */
extern void settilesetting(int ruleset, const char *filename);

/* Set the active SFX theme name (e.g. "MS", "LINK", or "" / "Tile World"). */
extern void setsfxtheme(const char *theme_name);

/* Return TRUE if active theme rc configured StopBgmOnLevelComplete=1. */
