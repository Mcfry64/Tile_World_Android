...
static void choosetileset(int activeruleset);
static char **scanbmpfiles(int *outcount);
static const char *pickbmpfile(const char *heading, const char *hint,
                                char **files, int count, const char *initial);

/* Bell-ringing macro.
 */
#define	bell()	(silence ? (void)0 : ding())

#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, "tworld", __VA_ARGS__)
...
typedef struct startupdata {
    ...
    char* tileset; /* a selected tileset bitmap */
...
static int endinput(gamespec* gs) {
    ...
    for (;;) {
        ...
        switch (cmd) {
            ...
            case CmdHelp: dohelp(Help_KeysBetweenGames);
                return TRUE;
            case CmdTileset: choosetileset(gs->series.ruleset);
                return TRUE;
...
#define SETPAUSED(paused, shutter) do { \
	...
	gamepaused = TRUE; \
	android_set_paused_title(1); \
    } \
    else { \
	setdisplaymsg(NULL, 0, 0); \
	setgameplaymode(NormalPlay); \
	gamepaused = FALSE; \
	android_set_paused_title(0); \
    } \
} while (0)
    ...
    for (;;) {
        ...
        if (!(cmd >= CmdMoveFirst && cmd <= CmdMoveLast)) {
            switch (cmd) {
                ...
                case CmdTileset:
                    setgameplaymode(SuspendPlay);
                    choosetileset(gs->series.ruleset);
                    if (!gamepaused) setgameplaymode(NormalPlay);
                    cmd = CmdNone;
                    break;
#ifndef NDEBUG
...
/* ── Tileset chooser ──────────────────────────────────────────────────────
 * Scans resdir for .bmp tile files, lets the user pick one for each ruleset,
 * rewrites the relevant [MS] / [Lynx] TileImages= lines in the rc file, and
 * reloads the tileset for the currently active ruleset.
 */

/* Scan resdir for *.bmp files that are not font.bmp.
 * Returns a malloc'd array of heap-allocated filenames (caller frees),
 * sorted alphabetically. *outcount is set to the number of entries.
 */
static char **scanbmpfiles(int *outcount)
{
    DIR *d = opendir(resdir);
    if (!d) { *outcount = 0; return NULL; }

    int cap = 16, n = 0;
    char **files = malloc(cap * sizeof *files);
    if (!files) { closedir(d); *outcount = 0; return NULL; }
    ...
    while ((ent = readdir(d)) != NULL) {
        ...
        if (tolower((unsigned char)ext[0]) != '.' ||
            ...
            tolower((unsigned char)ext[3]) != 'p') continue;
        if (strcasecmp(name, "font.bmp") == 0) continue;
...
/* Rewrite TileImages= for one section ("MS" or "Lynx") in resdir/rc. */
static void updaterctilesetting(const char *sectiontag, const char *newfile)
{
    char rcpath[512], tmppath[512];
    snprintf(rcpath,  sizeof rcpath,  "%s/rc",     resdir);
    snprintf(tmppath, sizeof tmppath, "%s/rc.tmp", resdir);

    FILE *fin = fopen(rcpath, "r");
    if (!fin) return;
    FILE *fout = fopen(tmppath, "w");
    if (!fout) { fclose(fin); return; }

    char buf[512];
    int in_target = 0, patched = 0;

    while (fgets(buf, (int)sizeof buf, fin)) {
        char secname[64];
        if (sscanf(buf, "[%63[^]]]", secname) == 1) {
            in_target = (strcasecmp(secname, sectiontag) == 0);
            fputs(buf, fout);
            continue;
        }
        if (in_target && !patched) {
            char key[128];
            if (sscanf(buf, "%127[^=]=", key) == 1) {
                char *k = key;
                while (isspace((unsigned char)*k)) k++;
                char *end = k + strlen(k);
                while (end > k && isspace((unsigned char)end[-1])) *--end = '\0';
                if (strcasecmp(k, "tileimages") == 0) {
                    fprintf(fout, "TileImages=%s\n", newfile);
                    patched = 1;
                    continue;
                }
            }
    ...
    fclose(fin);
    fclose(fout);
    rename(tmppath, rcpath);
}

/* Show a scrollable list of .bmp filenames, return chosen name or NULL. */
static const char *pickbmpfile(const char *heading, const char *hint,
                                char **files, int count, const char *initial)
{
    if (count == 0) return NULL;

    /* Build tablespec manually (PRODUCE_SINGLE_COLUMN_TABLE needs non-empty R). */
    size_t alloc = 3 + strlen(heading);
    for (int i = 0; i < count; i++)
        alloc += 3 + strlen(files[i]);
    char const **ptrs = malloc((count + 1) * sizeof *ptrs);
    char *heap = malloc(alloc);
    if (!ptrs || !heap) memerrexit();

    int used = 0, idx = 0;
    ptrs[idx++] = heap + used;
    used += 1 + sprintf(heap + used, "1-%s", heading);
    for (int i = 0; i < count; i++) {
        ptrs[idx++] = heap + used;
        used += 1 + sprintf(heap + used, "1-%s", files[i]);
    }
    tablespec tbl = { count + 1, 1, 0, 0, ptrs };

    int n = 0;
    for (int i = 0; i < count; i++)
        if (strcasecmp(files[i], initial) == 0) { n = i; break; }

    int f = displaylist(hint, &tbl, &n, LIST_MAPFILES, scrollinputcallback);
    free((void *)ptrs[0]);
    free(ptrs);

    return (f == CmdProceed && n >= 0 && n < count) ? files[n] : NULL;
}

static void choosetileset(int activeruleset)
{
    int count = 0;
    char **files = scanbmpfiles(&count);
    if (count == 0) {
        if (files) free(files);
        return;
    }

    const char *cur_ms   = gettilesetting(Ruleset_MS);
    const char *cur_lynx = gettilesetting(Ruleset_Lynx);

    pushsubtitle("Tileset - MS");
    const char *ms_pick = pickbmpfile("MS Tiles", "A: Select   B: Back",
                                      files, count, cur_ms);
    char *ms_selection = ms_pick ? strdup(ms_pick) : NULL;
    popsubtitle();
    if (ms_selection) {
        updaterctilesetting("MS", ms_selection);
        settilesetting(Ruleset_MS, ms_selection);
    }

    pushsubtitle("Tileset - Lynx");
    const char *lynx_pick = pickbmpfile("Lynx Tiles", "A: Select   B: Back",
                                        files, count, cur_lynx);
    char *lynx_selection = lynx_pick ? strdup(lynx_pick) : NULL;
    popsubtitle();
    if (lynx_selection) {
        updaterctilesetting("Lynx", lynx_selection);
        settilesetting(Ruleset_Lynx, lynx_selection);
    }

    for (int i = 0; i < count; i++) free(files[i]);
    free(files);

    if (ms_selection || lynx_selection)
        loadgameresources(activeruleset);
...
static int chooseseries(seriesdata* series, int* pn, int founddefault) {
    ...
    while (chosenseries < 0) {
        int f = displaylist("A: Select   X: Scores   Y: Password   B: Back", &mftable, &n, LIST_MAPFILES, scrollinputcallback);
        if (f == CmdTileset) {
            choosetileset(getselectedruleset());
            continue;
        }
        if (f != CmdProceed) {
            free_table(&mftable);
        ...
        if (chosengsl->count == 1)
            ...
            int m = 0;
            for (;;) {
                f = displaylist("A: Select   B: Back", &gstable, &m, LIST_SERIES, scrollinputcallback);
                if (f == CmdProceed) {
...
static int selectseriesandlevel(gamespec* gs, seriesdata* series, int autosel,
    ...
    okay = TRUE;
    if ((series->count == 1 && autosel) || (defaultlevel > 0)) {
    ...
    if (defaulttileset && *defaulttileset) {
        settilesetting(Ruleset_MS, defaulttileset);
        settilesetting(Ruleset_Lynx, defaulttileset);
    }
    ...
    if (defaultlevel) {
        ...
        if (n >= 0) {
            gs->currentgame = n;
            // Explicitly requested level via launcher bypasses the password check
            // and marks the password as "seen" so the engine is happy.
            passwordseen(gs, n);
        }
    }
...
static int choosegame(gamespec* gs, char const* lastseries) {
    ...
    if (!createserieslist(NULL, &s.list, &s.count, &s.mflist, &s.mfcount,
                          &s.table))
        return -1;
    return selectseriesandlevel(gs, &s, FALSE, lastseries, 0, NULL);
}
...
static int initoptionswithcmdline(int argc, char* argv[], startupdata* start) {
    ...
    *start->filename = '\0';
    ...
    initoptions(&opts, argc - 1, argv + 1, "abD:dFfHhL:lm:n:PpqR:rS:stT:Vv:c");
...
static void shutdownsystem(void) {
    ...
    shutdowngamestate();
    freeallresources();
    free(resdir); resdir = NULL;
    free(seriesdir); seriesdir = NULL;
    free(seriesdatdir); seriesdatdir = NULL;
...
static int choosegameatstartup(gamespec* gs, char const* lastseries,
    ...
    LOGI("choosegameatstartup: filename=%s levelnum=%d tileset=%s",
         start->filename ? start->filename : "NULL", start->levelnum,
         start->tileset ? start->tileset : "NULL");
    ...
    LOGI("choosegameatstartup: createserieslist found %d series", series.count);
    ...
    if (series.count == 1 || start->levelnum > 0)
        readextensions(series.list);

    return selectseriesandlevel(gs, &series, TRUE, lastseries, start->levelnum, start->tileset);
}
...
int tworld(int argc, char* argv[]) {
    ...
    int started_via_launcher = (start.filename && *start.filename);
    ...
    while (f > 0) {
        ...
        // If we started from the Android launcher, exit immediately
        // so we return to the launcher instead of showing the retro menu.
        if (started_via_launcher) {
